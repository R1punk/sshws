...
#install ssh ovpn
wget https://raw.githubusercontent.com/WaanStore/sshws/master/ssh-vpn.sh && chmod +x ssh-vpn.sh && screen -S ssh-vpn.sh ./ssh-vpn.sh
...
echo ""  | tee -a log-install.txt
echo "   - Dev/Main            : WaanStore"  | tee -a log-install.txt
...
echo ""  | tee -a log-install.txt
echo "------------------Script By WaanStore-----------------" | tee -a log-install.txt
echo ""
echo " Reboot 10 Sec"
sleep 10
rm -f setup.sh
